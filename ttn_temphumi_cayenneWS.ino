...
void do_send(osjob_t* j) {
  ...
  int16_t t_value, h_value, s_value;
    ...
    humi=constrain(humi,20,100);    //humi in range 20 to 100 (80 steps)
       ...
       Payload.addRelativeHumidity(1, humi);

   */
// FORMAT Data for Cayenne Platform
...
int16_t help;
 ...
 byte buf[20];
  buf[cursor++] = 0x03;
  buf[cursor++] = LPP_TEMPERATURE;
  help = temp * 10;
  buf[cursor++] = help >> 8;
  buf[cursor++] = help;
 
  buf[cursor++] = 0x04;
  buf[cursor++] = LPP_RELATIVE_HUMIDITY;
  buf[cursor++] = humi; 
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    // Prepare upstream data transmission at the next possible time.
      LMIC_setTxData2(1, buf, cursor, 0); 
